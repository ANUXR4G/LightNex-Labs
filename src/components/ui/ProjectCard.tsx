...
const ProjectCard = ({ title, items, image, href }: ProjectCardProps) => {
  return (
    <Link href={href} target="blank" className="w-full border-y block border-[#3e393eb7] group text-[#c8c2bd]">
      <div className="flex xl:justify-between transition-all w-11/12 mx-auto flex-col xl:flex-row">
        <div className="flex items-start xl:items-center gap-x-8 justify-start xl:-ml-8 xl:group-hover:ml-0 flex-col xl:flex-row">
          <div className="relative w-full h-[200px] xl:w-[200px] xl:h-[200px]">
            <Image
              src={image}
              fill
          ...
          <h3 className="text-[30px] sm:text-[50px] xl:text-[70px] xl:-translate-x-[300px] xl:group-hover:-translate-x-0 duration-700 xl:py-8">
            {title}
          </h3>
        </div>
